  ...
  const getTrackingData = async () => {
    ...
    Promise.all([promise]).then((data) => {
      ...
      for (let i = 0; i < trackingissue.length; i++) {
        let element = trackingissue[i].Escalation
      ...
      trackingissue.map((res) => {
        for (let i = 0; i < mainElee.length; i++) {
          for (let j = 0; j < res.Escalation.length; j++) {
            let matchDate = mainElee[i].issueCreated === res.Escalation[j].issueCreated
